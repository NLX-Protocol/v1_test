...
import { convertTokenAddress, getNativeToken } from "config/tokens";
...
export async function createWithdrawalTxn(chainId: number, signer: Signer, p: Params) {
  ...
  const isNativeWithdrawal = isAddressZero(p.initialLongTokenAddress) || isAddressZero(p.initialShortTokenAddress)
    || p.initialLongTokenAddress === getNativeToken(chainId).wrappedAddress
    || p.initialShortTokenAddress === getNativeToken(chainId).wrappedAddress;
