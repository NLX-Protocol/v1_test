...
import { applyFactor, expandDecimals, formatAmount, formatBonk, formatUsd } from "lib/numbers";
...
export function formatBonkLiquidationPrice(liquidationPrice?: BigNumber, opts: { displayDecimals?: number } = {}) {
  if (!liquidationPrice || liquidationPrice.lte(0)) {
    return "NA";
  }

  return formatBonk(liquidationPrice, { ...opts, maxThreshold: "1000000" });
}
...
export function formatBonkAcceptablePrice(acceptablePrice?: BigNumber, opts: { displayDecimals?: number } = {}) {
  if (acceptablePrice && (acceptablePrice.eq(0) || acceptablePrice.gte(ethers.constants.MaxInt256))) {
    return "NA";
  }

  return formatBonk(acceptablePrice, { ...opts });
}
