...
export function formatBonk(
  usd?: BigNumber,
  opts: { fallbackToZero?: boolean; displayDecimals?: number; maxThreshold?: string; minThreshold?: string } = {}
) {
  const { fallbackToZero = false, displayDecimals = 6 } = opts;
  ...
  // const exceedingInfo = getLimitedDisplay(usd, USD_DECIMALS, opts);
  const sign = usd.lt(0) ? "-" : "";
  const displayUsd = formatAmount(usd, USD_DECIMALS, displayDecimals, true);
  return `${sign}$${displayUsd}`;
}
...
export function formatTokenAmountWithBtc(
  tokenAmount?: BigNumber,
  usdAmount?: BigNumber,
  tokenSymbol?: string,
  tokenDecimals?: number,
  ...
  const usdStr = formatTokenAmount(usdAmount, USD_DECIMALS,"BTC", { ...opts, useCommas: true, displayDecimals });
  // const usdStr = formatUsd(usdAmount, {
  //   fallbackToZero: opts.fallbackToZero,
  // });
  return `${tokenStr} (${usdStr})`;
...
export function formatTokenAmountBuySell(
  tokenAmount?: BigNumber,
  usdAmount?: BigNumber,
  firstSymbol?: string,
  tokenSymbol?: string,
  ...
  const tokenStr = formatTokenAmount(tokenAmount, 18, tokenSymbol, { ...opts, useCommas: true });

  const usdStr = formatTokenAmount(usdAmount, USD_DECIMALS,firstSymbol, { ...opts, useCommas: true, displayDecimals });
  // const usdStr = formatUsd(usdAmount, {
  //   fallbackToZero: opts.fallbackToZero,
  // });
  return `${tokenStr} (${usdStr})`;
