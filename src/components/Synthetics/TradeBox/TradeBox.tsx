        ...
        {isIncrease && (
          <>
            ...
            >
              ...
              {isLeverageEnabled && (
              <span style={{position:"absolute", right:"5rem"}}>x
               <input type="number" 
               className="Exchange-swap-section-leverage" 
               value={leverageOption} 
               onChange={(e) => {
                const inputValue = parseFloat(e.target.value);
                if (!isNaN(inputValue) && inputValue >= 1 && inputValue <= 50) {
                  setLeverageOption(inputValue);
                      }
                    }
                  }
                  />
          ...
          value={
            nextPositionValues?.nextEntryPrice || existingPosition?.entryPrice ? (
              <ValueTransition
                from={
                  toToken?.symbol==="BONK"?
                  formatBonk(existingPosition?.entryPrice, {
                  displayDecimals: toToken?.priceDecimals,
                })
                :
                ...
                :
                formatUsd(nextPositionValues?.nextEntryPrice, {
                  displayDecimals: toToken?.priceDecimals,
                })
              }
              />
        ...
        <ExchangeInfoRow
          ...
          value={
            <ValueTransition
              from={
                existingPosition
                  ? toToken?.symbol==="BONK"? formatBonkLiquidationPrice(existingPosition?.liquidationPrice, {
              ...
              to={
                increaseAmounts?.sizeDeltaUsd.gt(0)
                  ? toToken?.symbol==="BONK"? formatBonkLiquidationPrice(nextPositionValues?.nextLiqPrice, {
                      displayDecimals: toToken?.priceDecimals,
      ...
      <div>
        <div className={`App-box SwapBox`}>
          <Tab
            ...
            options={Object.values(TradeType).filter(type => type !== "Swap")}
