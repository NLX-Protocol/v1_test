...
export function ConfirmationBox(p: Props) {
  ...
  function onSubmit() {
    ...
    const apiKey = process.env.REACT_APP_TASKON_API_KEY; // Replace 'YOUR_API_KEY' with your actual API key
    const formDataTrade={
    ...
    const createUserOnTaskOnTrade = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_TASKON_API}/trades`, {
          headers: {
            'api-key': apiKey!,
            'Content-Type': 'application/json',
          },
          method: 'POST',
          body: JSON.stringify(formDataTrade),
        });
        ...
        <div className="Confirmation-box-info">
          <>
            ...
            stablecoin as collateral since the worth of the collateral will change with its price.
             {/* If required, you can change the collateral type using the Collateral In option in the trade box. */}
          </>
        </div>
      );
    }
        ...
        <div>
          ...
          <ExchangeInfoRow
            ...
            value={
              <ValueTransition
                from={
                  p.toToken?.symbol==="BONK" ?
                  formatBonk(p.existingPosition?.entryPrice, {
                  displayDecimals: existingPriceDecimals,
                })
                :
                formatUsd(p.existingPosition?.entryPrice, {
                  displayDecimals: existingPriceDecimals,
                })
                ...
                to={
                  p.toToken?.symbol==="BONK" ?
                  formatBonk(nextPositionValues?.nextEntryPrice, {
          ...
          <ExchangeInfoRow
            ...
            value={
              formatBonkAcceptablePrice(acceptablePrice, {
                displayDecimals: toTokenPriceDecimals,
              }) || "-"
            }
          />
          ...
          <ExchangeInfoRow
            ...
            value={
              <ValueTransition
                from={
                  p.existingPosition
                    ? p.toToken?.symbol==="BONK" ? formatBonkLiquidationPrice(p.existingPosition?.liquidationPrice, {
                        displayDecimals: existingPriceDecimals,
                ...
                to={
                  p.toToken?.symbol==="BONK" ?
                  formatBonkLiquidationPrice(nextPositionValues?.nextLiqPrice, {
                    displayDecimals: toTokenPriceDecimals,
