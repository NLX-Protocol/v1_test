...
import { formatAmount, formatBonk, formatPercentage, formatUsd, getBasisPoints } from "lib/numbers";
...
export function MarketCard({ marketInfo, allowedSlippage, isLong, isIncrease }: Props) {
  ...
  const {
    ...
    return {
      ...
      borrowingRate: getBorrowingFactorPerPeriod(marketInfo, isLong, CHART_PERIODS["funding_fee"]).mul(100),
      fundingRateLong: getFundingFactorPerPeriod(marketInfo, true, CHART_PERIODS["funding_fee"]).mul(100),
      fundingRateShort: getFundingFactorPerPeriod(marketInfo, false, CHART_PERIODS["funding_fee"]).mul(100),
  ...
  const renderAnnualizedFundingFeeTooltipContent = useCallback(() => {
    if (!fundingRateLong || !fundingRateShort) return [];
    const isAdaptiveFundingForMarketActive = marketInfo && isMarketAdaptiveFundingActive(marketInfo);
    ...
    const long = (
      <>
        ...
        <span className={isLongPositive ? "text-green" : "text-red"}>
          {isLongPositive ? "+" : "-"}
          {formatAmount((fundingRateLong.abs()).mul(24*365), 30, 4)}%
        </span>{" "}
        annually.
      </>
    );
    ...
    const short = (
      <>
        ...
        <span className={isShortPositive ? "text-green" : "text-red"}>
          {isShortPositive ? "+" : "-"}
          {formatAmount((fundingRateShort.abs()).mul(24*365), 30, 4)}%
        </span>{" "}
        annually.
      </>
    );
        ...
        {isAdaptiveFundingForMarketActive && (
          <span>
            ...
            <Trans>
              This market uses an Adaptive Funding Rate. The Funding Rate will adjust over time depending on the ratio
              of longs and shorts.{" "}
              {/* <ExternalLink href="https://docs.nlx.trade">Read more</ExternalLink>. */}
            </Trans>
          </span>
        )}
      </div>
    ...
    <div className="Exchange-swap-market-box App-box App-box-border MarketCard">
      ...
      <div>
        ...
        <ExchangeInfoRow
          ...
          value={
            <Tooltip
              handle={indexToken?.symbol==="BONK"? formatBonk(entryPrice, { displayDecimals: priceDecimals }): formatUsd(entryPrice, { displayDecimals: priceDecimals }) || "..."}
              ...
              renderContent={() => (
                <>
                  ...
                  <br />
                  {/* <br /> */}
                  {/* <ExternalLink href="https://docs.nlx.trade">More Info</ExternalLink> */}
                </>
              )}
            />
          }
        ...
        <ExchangeInfoRow
          ...
          value={
            <Tooltip
              handle={
                indexToken?.symbol==="BONK"?
                formatBonk(exitPrice, {
        ...
        <ExchangeInfoRow
          label={t`Annualized Funding Fee`}
          value={
            <Tooltip
              className="al-swap"
              handle={
                fundingRate ? `${fundingRate.gt(0) ? "+" : "-"}${formatAmount((fundingRate.abs()).mul(24*365), 30, 4)}%` : "..."
              }
              position="right-bottom"
              renderContent={renderAnnualizedFundingFeeTooltipContent}
            />
          }
        />

        <ExchangeInfoRow
          label={t`Hourly Funding Fee`}
          value={
            <Tooltip
              className="al-swap"
              handle={
