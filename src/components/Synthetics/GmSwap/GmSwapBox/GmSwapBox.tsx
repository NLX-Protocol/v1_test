...
export function GmSwapBox(p: Props) {
  ...
  const operationLabels = {
    [Operation.Deposit]: t`Buy NX`,
    [Operation.Withdrawal]: t`Sell NX`,
  };
  ...
  const { longTokenInputState, shortTokenInputState } = useMemo(() => {
    ...
    if (firstTokenAddress && !marketInfo.isSameCollaterals) {
      inputs.push({
    ...
    if (isDeposit && firstTokenAddress && marketInfo.isSameCollaterals) {
      inputs.push({
        address: firstTokenAddress,
        value: firstTokenInputValue ,
        setValue: setFirstTokenInputValue,
        amount: firstTokenAmount?.div(2),
        usd: firstTokenUsd?.div(2),
        token: firstToken,
      });
    }
    ...
    if (marketInfo.isSameCollaterals) {
      return {
        longTokenInputState: inputs[0],
        shortTokenInputState: inputs[0],
      };
    }
  ...
  const submitState = useMemo(() => {
    ...
    return {
      text: isDeposit ? t`Buy NX` : t`Sell NX`,
      onSubmit,
    };
  }, [
    account,
      ...
      >
        <div className={cx("GmSwapBox-form-layout", { reverse: isWithdrawal })}>
          <BuyInputSection
            ...
            inputValue={isDeposit ? firstTokenInputValue : Number(firstTokenInputValue)+Number(secondTokenInputValue)}
