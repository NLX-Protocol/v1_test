...
import { DEFAULT_HIGHER_SLIPPAGE_AMOUNT, EXCESSIVE_SLIPPAGE_AMOUNT } from "config/factors";
...
export function GmConfirmationBox({
  ...
  const tokensToApprove = (function getTokensToApprove() {
    ...
    const longShortTokenText = marketTokenAmount?.gt(0)
    ? formatTokenAmountWithUsd(longTokenAmount?.mul(2), marketTokenUsd, ("Long/Short"), shortToken?.decimals)
    : undefined;
  ...
  const operationText = isDeposit ? t`Stake` : t`Unstake`;
  ...
  const submitButtonState = (function getSubmitButtonState() {
    ...
    const onSubmit = () => {
      ...
      const apiKey = process.env.REACT_APP_TASKON_API_KEY; 
      const formDataEarn={
      ...
      const createUserOnTaskOnEarn = async () => {
        try {
          const response = await fetch(`${process.env.REACT_APP_TASKON_API}/earn`, {
            headers: {
              'api-key': apiKey!,
              'Content-Type': 'application/json',
            },
            method: 'POST',
            body: JSON.stringify(formDataEarn),
          });
    ...
    if (isSubmitting) {
      return {
        text: isDeposit ? t`Staking...` : t`Unstaking...`,
        disabled: true,
      };
    }
    ...
    if (tokensToApprove.length > 0 && marketToken) {
      const symbols = tokensToApprove.map((address) => {
        ...
        return address === marketToken.address ? "NX" : token?.assetSymbol ?? token?.symbol;
      });
    ...
    const operationText = isDeposit ? t`Stake` : `Unstake`;
  ...
  function onCreateDeposit() {
    ...
    return createDepositTxn(chainId, signer, {
      ...
      allowedSlippage: EXCESSIVE_SLIPPAGE_AMOUNT,
        ...
        {isVisible && (
          <>
            <div className={cx("Confirmation-box-main GmConfirmationBox-main")}>
              {isDeposit && (
                <>
                  {[longShortTokenText].filter(Boolean).map((text) => (
                    <div key={text}>
                  ...
                  return (
                    <div key={address}>
                      <ApproveTokenButton
                        ...
                        tokenSymbol={address === marketToken?.address ? "NX" : token.assetSymbol ?? token.symbol}
                        spenderAddress={routerAddress}
                      />
                    </div>
                  );
